...
Automated content moderation reduces the cost of moderating images, videos, text, and voice by accurately detecting inappropriate content.

Jarvis Moderator provides services through open application programming interfaces (APIs). You can obtain inference results by calling these APIs, which helps you build an intelligent service system and improve efficiency.
...
| data          | Object   | <p align="center">`{ "_id": "", "status": 1 }`</p> | `_id`: Interface returns data, status: the status of video： [1:queueing, 2:processing, 3:completed, 4:failed] |
