...
const detectFace = async (token, imageUrl) => {
    const response = await fetch('https://openapi.akool.com/interface/detect-api/detect_faces', {
        method: 'POST',
        headers: {
            'x-api-key: {{API Key}}',
            'Content-Type': 'application/json'
        ...
        body: JSON.stringify({
            "url": imageUrl, // Supports image, video, and GIF formats
            "single_face": false,
            "return_face_url": true, // Whether to return the cropped face URL
        })
    });
...
def detect_face(token, image_url):
    ...
    payload = {
        "url": image_url, // Supports image, video, and GIF formats
        "single_face": false,
        "return_face_url": true // Whether to return the cropped face URL
    }
    headers = {
