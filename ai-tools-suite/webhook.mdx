...
**Webhook is an HTTP-based callback function that allows lightweight, event-driven communication between two application programming interfaces (APIs). Webhooks are used by a wide variety of web apps to receive small amounts of data from other apps.**

**Response Data (This is the response data that the webhookUrl in the request parameter needs to return.)**
<Note> If success, http statusCode it must be 200</Note>
- **statusCode** is the HTTP status of the response to your request. If successful, it must be **200**. If you do not return a status code of 200, we will retry sending the response to your webhook URL.
...
| status        | Number   | <p align="center">2  or  3  or 4</p>                                                 | status: the status of  image or video or faceswap or background change or avatar or   audio： 【1:queueing, 2:processing,3:completed, 4:failed】 |
| type          | String   | <p align="center">faceswap or image or audio or talking photo or video translate or background change or avatar or lipsync</p> | Distinguish the type of each interface     |
